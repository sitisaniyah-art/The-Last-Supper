...
- **管理员后台** — GitHub Token 认证的数据管理面板（含图片管理 + 评论举报 + 数据统计）
- **滚动公告栏** — 页面顶部实时动态展示
...
├── admin-dashboard-xk7m2q/index.html  # 管理后台页（随机路径）
...
### 8.8 `assets/js/analytics.js`

**职责：** 本地数据分析模块（纯本地，无外部 API 调用）

```javascript
TLSAnalytics.trackPageView()               // 追踪页面访问
TLSAnalytics.trackClick(element, page)     // 追踪按钮点击
TLSAnalytics.trackResourceView(resourceId) // 追踪资源查看
TLSAnalytics.getStats()                    // 获取统计摘要
TLSAnalytics.getPageViews()                // 获取页面访问记录
TLSAnalytics.getClicks()                   // 获取点击记录
TLSAnalytics.getResourceViews()            // 获取资源查看记录
```

- 存储 key：`tls_analytics`
- 追踪：页面访问、按钮点击、资源查看、会话时长
- 每 30 秒批量保存 + `beforeunload` 时保存
- Bot 过滤：检测 bot/crawler/lighthouse/headless 等 UA
- 每个数组上限 500 条，自动裁剪
- 自动追踪 `.download-btn`, `.favorite-btn`, `.zj-like-btn` 等按钮点击
- 管理后台"数据统计"Tab 读取此数据展示
...
访问 `/admin-dashboard-xk7m2q/` 页面，使用 GitHub Personal Access Token（PAT）进行验证。

**认证流程：**
1. 输入具有 `repo` 权限的 GitHub Token
2. 验证 Token 有效性（`GET /user`）
3. 检查用户是否为项目协作者（`GET /repos/{owner}/{repo}/collaborators/{username}`）
4. 允许的用户列表：`sitisaniyah-art`（硬编码白名单）
5. Token 存储在 sessionStorage，关闭浏览器后自动清除

**安全机制：**
- 前端锁定：5 次验证失败后锁定 24 小时（localStorage `tls_admin_lock`）
- Token 仅存 sessionStorage，不持久化到 localStorage
- 页面锁定状态在加载时检查并显示剩余时间

**功能 Tab：**
- **数据概览** — 资源总数、下载量、平均评分、贡献者数、分类分布
- **举报管理** — 读取 GitHub 上带 `report` label 的 Issues（使用 Token 认证提升 API 限额）
- **操作日志** — 显示 `_data/logs.yml` 中的操作记录
- **资源管理** — 搜索和查看所有资源
- **嗨玩项目** — 管理嗨玩榜项目图片（编辑/上传/删除），支持 Imgbb 上传和 URL 粘贴
- **评论举报** — 查看和处理用户提交的评论举报（本地存储）
- **数据统计** — 本地 TLSAnalytics 数据展示（页面访问、按钮点击、资源查看、会话时长、热门资源排行）
